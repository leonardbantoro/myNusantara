...
const Home = ({ selectedProvince, handleProvinceClick }) => {
  ...
  useEffect(() => {
    const fetchWikiDescription = async () => {
      try {
        ...
        const truncatedDescription = truncateAtSecondPeriod(description);
        setWikiDescription(truncatedDescription);
      } catch (error) {
    ...
    const truncateAtSecondPeriod = (text) => {
      ...
      return secondPeriodIndex !== -1 ? text.substring(0, secondPeriodIndex + 1) : text;
    };
    ...
    const fetchWeatherInfo = async () => {
      try {
        const lat = -7.7956;
        const lon = 110.3695;
        const apiKey = '36d175a5e93238c7677781954e165216'; // Ganti dengan API key OpenWeatherMap Anda
        const response = await axios.get(
    ...
    const fetchSpotifyTrack = async () => {
      try {
        const clientId = '7d966a26c9ad4f108c1adac60ad5271d'; // Ganti dengan Client ID Spotify Anda
        const clientSecret = '4058dacccce249b79de0a6755f8e3565'; // Ganti dengan Client Secret Spotify Anda
        ...
        const trackId = '5KXAm75wLQr1MVtcDtJY0X'; // Ganti dengan track ID yang diinginkan
        ...
        setSpotifyTrack(trackResponse.data);
      } catch (error) {
    ...
    <Navbar/>
        ...
        <g transform="translate(20, 29)">
          {provinces.map((province, index) => (
            <Link to={province.id === 'jawa' ? '/yogya' : ''} key={province.id}>
              <path
                id={province.id}
                fill={province.id === 'jawa' ? (province.clickable && province.name === selectedProvince ? '#69C53C' : '#85A1C6') : '#85A1C6'}
                d={province.d}
      ...
      <div className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        <div className="sidebar-content">
          <h2>Yogyakarta</h2>
          <img src="yogyakarta.jpg" alt="Yogyakarta" className="sidebar-image" />
  ...
  <div className="spotify-track">
    ...
    <div className="track-container">
      <img src={spotifyTrack.album.images[0].url} alt={spotifyTrack.name} className="track-album-image" />
      <div className="track-info">
        <p><strong>{spotifyTrack.name}</strong> by {spotifyTrack.artists[0].name}</p>
